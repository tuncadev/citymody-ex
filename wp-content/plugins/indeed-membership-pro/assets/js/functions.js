...
function ihcPaymentGatewayUpdate(v, is_r){
  ...
  jQuery('#ihc_authorize_r_fields').css( 'display', 'none' );
	jQuery('#ihc_braintree_r_fields').css( 'display', 'none' );
  ...
  switch (v){
  	...
  	jQuery('#ihc_submit_bttn').off('click');
        ...
        jQuery('#ihc_authorize_r_fields').css( 'display', 'block' );
			}
		break;
		case 'braintree':
			//jQuery('#ihc_braintree_r_fields').fadeIn(200);
      jQuery('#ihc_braintree_r_fields').css( 'display', 'block' );
		break;
	}
}
		...
		jQuery.ajax({
          ...
          var promise = new Promise(function(resolve, reject) {
              var f = jQuery('.ihc-form-create-edit [name=ihc_state]');
    		    	var p = f.parent();
              if ( p.children('.ihc-register-notice').length > 0 ){
                  p.children('.ihc-register-notice').remove();
              }
    		    	p.append(r);
              resolve();
          });
    		...
    		if (e.val()){
    			jQuery.ajax({
    			    ...
    			    success: function (r) {
    			      ...
    			      setTimeout( function(){
                    //jQuery('#ihc_coupon_code_check_div_msg').fadeOut(500, function(){this.remove();});
                    jQuery('#ihc_coupon_code_check_div_msg').remove();
                }, 5000 );
    			    }
    			});
    		}
