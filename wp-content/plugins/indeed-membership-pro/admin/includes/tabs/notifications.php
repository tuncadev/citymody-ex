					...
					<select name="notification_type" id="notification_type" class="ump-js-change-notification-type iump-form-select ihc-form-element ihc-form-element-select ihc-form-select">
						<?php
							foreach ($notification_arr as $k=>$v){
								...
								?>
								<option value="<?php echo $k;?>" <?php echo ($meta_arr['notification_type']==$k) ? 'selected' : ''; ?>><?php echo $v;?></option>
								<?php
								switch($k){
									case 'admin_user_register':
					...
					<select name="level_id" class="iump-form-select ihc-form-element ihc-form-element-select ihc-form-select">
						...
						if ($levels && count($levels)){
							foreach ($levels as $k=>$v){
								?>
									<option value="<?php echo $k;?>" <?php echo ($meta_arr['level_id']==$k) ? 'selected' : ''; ?>><?php echo $v['name'];?></option>
								<?php
							}
						}
						?>
							...
							<?php
								...
								<tr onmouseover="ihcDhSelector('#notify_tr_<?php echo $item->id;?>', 1);" onmouseout="ihcDhSelector('#notify_tr_<?php echo $item->id;?>', 0);">
									<td><?php
										...
										?>
										<div class ="ihc-buttons-rsp ihc-visibility-hidden" id="notify_tr_<?php echo $item->id;?>">
											<a class ="iump-btns" href="<?php echo $url.'&tab=notifications&edit_notification='.$item->id;?>"><?php esc_html_e('Edit', 'ihc');?></a> |
											<span class ="iump-btns ihc-delete-link ihc-js-admin-notifications-delete-notification" data-id="<?php echo $item->id;?>" ><?php esc_html_e('Delete', 'ihc');?></span>
										</div>
									</td>
