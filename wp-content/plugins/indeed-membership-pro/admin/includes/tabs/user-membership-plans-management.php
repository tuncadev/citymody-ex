...
<div class="ihc-admin-select-level-wrapper">
    ...
    <div class="iump-form-line">
        ...
        <select class="ihc-select-level" data-uid="<?php echo $uid;?>">
          ...
          <tbody>
            <?php if ( $userSubscriptions ):?>
              ...
              foreach ( $userSubscriptions as $subscription ):?>
                  ...
                  ?>
                  <tr class="<?php echo 'ihc-js-user-level-row-' . $subscription['level_id'];?> <?php if($i%2==0){
                    echo 'alternate';
                  } ?>">
                    <td class="ihc-levels-table-name">
                        <?php echo \Indeed\Ihc\Db\Memberships::getMembershipLabel( $subscription['level_id'] );?>
                        <input type="hidden" name="ihc_user_levels[]" class="ihc_user_levels_input" value="<?php echo $subscription['level_id'];?>" />
                    </td>
                    <td><?php
                    ...
                    <td>
                      <div class="input-group">
                        <input type="text" name="<?php echo 'start_time_levels[' . $subscription['level_id'] . ']';?>" value="<?php echo $subscription['start_time'];?>" placeholder="<?php echo $subscription['start_time'];?>" class="start_input_text form-control" />
                        <div class="input-group-addon"><i class="fa-ihc ihc-icon-edit"></i></div>
                     </div>
                    </td>
                    <td>
                      <div class="input-group">
                        <input type="text" name="<?php echo 'expire_levels[' . $subscription['level_id'] . ']';?>" value="<?php echo $subscription['expire_time'];?>" placeholder="<?php echo $subscription['expire_time'];?>" class="expire_input_text  form-control" />
                        <div class="input-group-addon"><i class="fa-ihc ihc-icon-edit"></i></div>
                     </div>
                    </td>
                    ...
                    <td>
                        ...
                        <?php if ( $subscription['expire_time'] == '0000-00-00 00:00:00' ):?>
                            ...
                            <div class="ihc-js-activate-user-level ihc-pointer" data-lid="<?php echo $subscription['level_id'];?>" ><?php esc_html_e( 'Activate', 'ihc' );?></div>
                        <?php endif;?>
                        ...
                        <?php if ( $subscriptionStatus['status'] != 4 && $subscription['expire_time'] != '0000-00-00 00:00:00' && strtotime( $subscription['expire_time'] ) < indeed_get_unixtimestamp_with_timezone() && empty( $remainTime ) ):?>
                            ...
                            <div class='ihc-js-renew-user-level ihc-pointer' data-lid='<?php echo $subscription['level_id'];?>' ><?php esc_html_e( 'Renew', 'ihc' );?></div>
                        <?php endif;?>

                        <?php if ( $membershipData['access_type'] === 'regular_period' && $subscription['status'] == 1 ):?>
                            <div title="<?php esc_html_e( 'First charging plan should be Canceled from Payment Service Platform', 'ihc' );?>" class="ihc-js-set-at-canceled-user-level ihc-pointer" data-lid="<?php echo $subscription['level_id'];?>"
                        ...
                        <?php endif;?>
                            ...
                            <div class='ihc-js-delete-user-level ihc-pointer' data-lid='<?php echo $subscription['level_id'];?>'><?php esc_html_e( 'Remove', 'ihc' );?></div>
