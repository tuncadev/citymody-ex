                        		...
                        		if (isset($posible_values[$value])){
                        			?>
		                        		<div id="ihc_select_tag_<?php echo $value;?>" class="ihc-tag-item">
		                        	    	...
		                        	    	<div class="ihc-remove-tag" onclick="ihcremoveTagForEditPost('<?php echo $value;?>', '#ihc_select_tag_', '#ihc_mb_who-hidden');" title="<?php esc_html_e('Removing tag', 'ihc');?>">x</div>
		                        	    </div>
		                        	<?php
                        		}
                        	}//end of foreach
	...
	<select class="ihc-fullwidth ihc-select" name="ihc_mb_block_type" onChange="ihcRedirectReplaceDd(this.value);">
		<?php
			foreach($select_types as $value=>$label){
				?>
					<option value="<?php echo $value;?>" <?php echo ($meta_arr['ihc_mb_block_type']==$value) ? 'selected' : ''; ?> >
						<?php echo $label;?>
					</option>
				<?php
			}
	...
	<select name="ihc_mb_redirect_to" class=" ihc-select ihc_mb_redirect_to">
		...
		<?php
			...
			if($default_redirect && $default_redirect!=-1){
				?>
				<option value="<?php echo $default_redirect;?>" <?php echo ($meta_arr['ihc_mb_redirect_to']==$default_redirect || $meta_arr['ihc_mb_redirect_to']==-1) ? 'selected' : ''; ?> >
					<?php
						...
						if ($title){
							...
							echo " ( " . esc_html__("Custom Link:", 'ihc') . $default_redirect . " )";
						}
					?>
				</option>
				<?php
			...
			foreach ($pages as $k=>$v){
				...
				?>
					<option value="<?php echo $k;?>" <?php echo ($meta_arr['ihc_mb_redirect_to']==$k) ? 'selected' : ''; ?> ><?php echo $v;?></option>
				<?php
				}
			}
		?>
...
?>
<div class="<?php echo $class;?>" id="ihc-meta-box-replace">
