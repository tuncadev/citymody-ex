    ...
    */
    ...
    public function subscriptionTable()
    {
        ...
        $data['uid'] = $uid;
  			...
  			/// STRIPE PAYMENT
  			if (!empty($_POST['stripeToken']) && (empty($_GET['ihc_register']) || $_GET['ihc_register']!='create_message') && !defined( 'IHC_PAYMENT_WITH_STRIPE_WAS_MADE' ) ){
    ...
    public function pushoverNotificationsForm()
    {
        ...
        $this->setSettings();
  			...
  			if (!empty($_POST['ihc_pushover_token']) && !empty( $_POST['ihc_pushover_nonce'] ) && wp_verify_nonce( (isset($_POST['ihc_pushover_nonce'])) ? $_POST['ihc_pushover_nonce'] : '', 'ihc_pushover_nonce' ) ){
    ...
    public function userSitesAddNewForm()
    {
      ...
      if (!empty($_POST['add_new_site']) && isset($_POST['lid'])
            ...
            && wp_verify_nonce( $_POST['ihc_multi_site_add_edit_nonce'], 'ihc_multi_site_add_edit_nonce' ) ) {
        $lid = $_POST['lid'];
    ...
    public function userBanner( $attr=[] )
    {
        ...
        if ( !$data['banner'] ){
            $data['banner'] = IHC_URL . 'assets/images/top_ump_bk_4.png'; /// default
        }
