...
class ChangePassword
{
    ...
    public function processing( $action='', $postData=[] )
    {
        ...
        if ( !$user || !wp_check_password( esc_sql( $_POST['current_pass'] ), $user->data->user_pass, $user->ID ) ){
        ...
        $newPass1Response = $validator->resetInputProperties()
                                      ...
                                      ->setCurrentValue( esc_sql( $postData['new_pass1'] ) )
                                      ->isValid();
        ...
        $newPass2Response = $validator->resetInputProperties()
                                      ...
                                      ->setCurrentValue( esc_sql( $postData['new_pass2'] ) )
                                      ->setCompareValue( esc_sql( $postData['new_pass1'] ) )
                                      ->isValid();
        ...
        $userData = [
                'ID'          => $uid,
                'user_pass'   => esc_sql( $postData['new_pass1'] ),
        ];
