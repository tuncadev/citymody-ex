		...
		public function form(){
				...
				if ( (isset($v[$this->display_type]) && $v[$this->display_type]>0) || $this->is_change_password){
					...
					switch ($v['name']){
						...
						case 'recaptcha':
							if ($this->captcha){
								...
								if ($disp_captcha != ''){
									...
									$str .= "</div>";
									$this->required_fields[] = array('name' => 'recaptcha', 'type' => 'recaptcha' );
								}
							}
							break;
						...
						case 'ihc_memberlist_accept':
								...
								if($v['name'] === 'ihc_optin_accept' && isset($v['ihc_optin_accept_checked'])
								&& $v['ihc_optin_accept_checked'] === '1'){
									$val = 1;
								}
								if($v['name'] === 'ihc_memberlist_accept' && isset( $v['ihc_memberlist_accept_checked'] )
								&& $v['ihc_memberlist_accept_checked'] === '1'){
									$val = 1;
								}
			...
			if ( !$this->disabled_submit_form){
				...
				if (isset($this->current_level) && $level_data!==false && $level_data['payment_type']=='payment'){
						if ( !ihcCheckCheckoutSetup() && ($this->payment_gateway == 'authorize' || !empty($include_authorize)) && ihc_check_payment_available('authorize') && isset($level_data['access_type']) && $level_data['access_type']=='regular_period'){
							//AUTHORIZE RECCURING - DEPRECATED
			...
			if ($this->type=='create'){
				$button_label = esc_html__('Register', 'ihc');
				$register_button_label = get_option('ihc_register_button_label');
				if(isset($register_button_label) && $register_button_label != ''){
					$button_label = $register_button_label;
				}
				$str .= indeed_create_form_element(array('type'=>'submit', 'name'=>'Submit', 'value' => $button_label,
						'class' => $bttn_submit_class, 'id'=>'ihc_submit_bttn', 'disabled'=>$this->disabled_submit_form, 'other_args' => ' data-standard-label="'.$button_label.'" data-loading-label="' . esc_html__( 'Please wait...', 'ihc') . '" ' ));
			} else {
				$button_label = esc_html__('Save Changes', 'ihc');
			...
			if ( $this->type=='create'){
				...
				if ( empty($this->is_modal) ){
					$js .= 'jQuery(".ihc-form-create-edit").on("submit", function(evt) {
		...
		$callback = '';
		 ...
		 if ($v['type']=='date'){
			...
			//OLD Workflow --> END
				...
				/// REQUIRE INPUT
			 	if (!empty($v['req']) || $v['type']=='conditional_text'
				|| $v['name']=='user_login' || $v['type'] == 'unique_value_text' ){
			 		  $this->required_fields[] = array( 'name' => $v['name'], 'type' => $v['type'] );
			 	}
				/// REQUIRE INPUT
		 ...
		 switch ($this->register_template){
			 ...
			 case 'ihc-register-12':
			  //////// FORM FIELD
			 	 ...
			 	 $temp_type_class = 'iump-form-' . $v['type'];
				 $str .= '<div class="iump-form-line-register ' . $temp_type_class . '" id="' . $parent_id . '">';
				 if ($v['type'] == 'text' || $v['type'] == 'password' || $v['type'] == 'ihc_invitation_code_field'){
				 	if ($v['req']){
						 $placeholder .= '*';
					 }
					if (isset($v['native_wp']) && $v['native_wp']){
