...
class IndeedForms
{
    ...
    public static function password( $attr=[] )
    {
        ...
        $attr = self::initiateFullAttr( $attr );
        $output = '';
        wp_register_script( 'ihc_passwordStrength', IHC_URL . 'assets/js/passwordStrength.js', ['jquery'], 11.3 );
    ...
    public static function ihc_state( $attr=[] )
    {
        ...
        // if ( !isset( $attr['is_edit'] ) || $attr['is_edit'] === false ){
    ...
    public static function unique_value_text( $attr=[] )
    {
        ...
        $output = '<input type="text" data-search-unique="true" onBlur="ihcCheckUniqueValueField(\'' . $attr['name'] . '\');" name="'.$attr['name'].'" id="'.$attr['id'].'" class="'.$attr['class'].'" value="' . ihc_correct_text( $attr['value'], false, true ) . '" placeholder="'.$attr['placeholder'].'" '.$attr['other_args'].' '.$attr['disabled'].' />';
        if (!empty($attr['sublabel'])){
            $output .= '<label class="iump-form-sublabel">' . ihc_correct_text($attr['sublabel']) . '</label>';
        }
        return $output;
    ...
    public static function ihc_dynamic_price( $attr=[] )
    {
        $attr = self::initiateFullAttr( $attr );
          ...
          $output .= "<input type='number' onChange='ihcDynamicPriceUpdateGlobal();' onBlur='ihcDynamicPriceUpdateGlobal();' min='$min' max='$max' class='{$attr['class']}' step='$step' value='$level_price' name='ihc_dynamic_price' id='ihc_dynamic_price' />";
          return $output;
    }
