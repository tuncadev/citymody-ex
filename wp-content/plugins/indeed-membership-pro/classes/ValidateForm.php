...
class ValidateForm
{
    ...
    public function ihc_invitation_code_field()
    {
        if ( $this->isRequired && $this->currentValue === '' ){
      	...
      	if ( $this->currentValue === '' || !\Ihc_Db::invitation_code_check( esc_sql( $this->currentValue ) ) ){
    ...
    public function checkConditionalLogic( $valueToCheck='' )
    {
        ...
        if ( $this->fieldData['conditional_logic_cond_type'] === 'has' ){
            //has value
            if ( $this->fieldData['conditional_logic_corresp_field_value'] == $valueToCheck ){
                return true;
            }
        } else {
            //contain value
            if ( strpos( $valueToCheck, $this->fieldData['conditional_logic_corresp_field_value'] ) !== false ){
                return true;
            }
        }
        return false;
    ...
    public function checkRecaptcha( $postData=[] )
    {
        ...
        if (isset($postData['g-recaptcha-response'])){
          ...
          if ( $secret ){
              ...
              $response = $recaptcha->verify($postData['g-recaptcha-response'], $_SERVER['REMOTE_ADDR']);
