...
wp_register_script( 'ihc-stripe-connect', IHC_URL . 'assets/js/stripe-connect.js', [ 'jquery' ], 11.4 );
...
<div class="ihc-stripe-connect-form-wrapper">
    ...
    <?php if ( get_option('ihc_stripe_connect_saved_cards') && isset( $paymentMethods ) && is_array( $paymentMethods ) && count( $paymentMethods ) ):?>
      ...
      <div class="ihc-stripe-connect-saved-cards">
            ...
            <?php foreach ( $paymentMethods as $paymentMethod ): ?>
                ...
                ?>
                <div class="ihc-stripe-connect-saved-card-wrapper <?php echo $extraClass;?>">
                    <input type="radio" checked name="ihc_stripe_connect_payment_methods" class="ihc-js-stripe-connect-paymnt-methods" value="<?php echo $paymentMethod['payment_method'];?>" />
                    <div class="ihc-stripe-connect-saved-card-details">
                      <span class="ihc-stripe-connect-saved-card-brand"><?php echo $paymentMethod['brand'];?></span>
                      <span class="ihc-stripe-connect-saved-card-number"><?php echo'**** ' . $paymentMethod['last4'];?></span>
                      <span class="ihc-stripe-connect-saved-card-exp"><?php echo $paymentMethod['exp_month'] . '/' . $paymentMethod['exp_year'];?></span>
                    </div>
                </div>
            <?php endforeach;?>
    ...
    <div class="ihc-js-stripe-connect-wrapp <?php echo $extraClassWrapp;?>" >
        <div class="ihc-stripe-connect-form-card-name-wrap" >
            <input type="text" name="ihc_stripe_connect_full_name" class="ihc-stripe-connect-input ihc-js-stripe-connect-full-name" value="<?php echo $fullName;?>"  placeholder="<?php esc_html_e('Name on the Card *', 'ihc');?>"/>
        </div>
