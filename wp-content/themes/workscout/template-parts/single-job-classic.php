						...
						<div>
							<strong>' . ($expired ? __('Closed', 'workscout') : __('Closes', 'workscout')) . ':</strong><span>' . date_i18n(get_option('date_format'), strtotime($deadline)) . '</span></div></li>';
								...
								<?php if (in_array("salary", $overview_elements)) : ?>
									...
									// if (is_numeric($price)) {
									// 	$decimals = get_option('hireo_number_decimals', 2);
							...
							if (candidates_can_apply()) {
								...
								if (!empty($apply)) {
									if ($apply->type == 'url') {
										echo '<a class="button margin-top-20" target="_blank" href="' . esc_url($apply->url) . '">' . esc_html__('Apply for job', 'workscout') . '</a>';
									} else {
									...
									if (!empty($apply)) {
										echo '<a class="button margin-top-20" target="_blank" href="' . esc_url($apply) . '">' . esc_html__('Apply for job', 'workscout') . '</a>';
									}
								}
							} ?>
