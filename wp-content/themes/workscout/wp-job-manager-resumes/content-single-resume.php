<?php if ( resume_manager_user_can_view_resume( $post->ID ) ) : ?>
	...
	<?php do_action( 'single_resume_start' ); ?>
		...
		<div class="eight columns">
			...
			<?php endif; ?>
						<?php if ( $items = get_post_meta( $post->ID, '_candidate_color', true ) ) : ?>
				<h3 class="margin-bottom-20 margin-top-20"><?php esc_html_e( 'Favourite Color', 'workscout' ); ?></h3>
				<dl class="resume-table resume-manager-experience">


						<dt>
							<small class="date"><?php echo get_post_meta( $post->ID, '_candidate_color', true ); ?></small>
							<strong><?php echo get_post_meta( $post->ID, '_candidate_color', true ); ?></strong> 
						</dt>
						<dd>
